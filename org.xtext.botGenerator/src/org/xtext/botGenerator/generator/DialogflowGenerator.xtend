...
class DialogflowGenerator {
	...
	def doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		if (isPlugin) {
			...
			uri = workspacePath+"/"+fsa.getURI(path).toPlatformString(true);
		}else{
			uri = newpath+"/"+path;
		}

		zip = new CreateZip(uri, bot.name)
		zip.addFile(uri, "package.json")
		...
		for (Entity entity : entities) {
			...
			zip.addFileToFolder(uri + '/entities/', 'entities', entity.name + '.json')
			...
			zip.addFileToFolder(uri + '/entities/', 'entities',
				entity.name + '_entries_' + lan.languageAbbreviation + '.json')
		}
	...
	def void createTransitionFiles(UserInteraction transition, String prefix, IFileSystemAccess2 fsa, Bot bot) {
		...
		zip.addFileToFolder(uri + '/intents/', 'intents', prefix + transition.intent.name + '.json')
		...
		for (IntentLanguageInputs input : transition.intent.inputs) {
			...
			zip.addFileToFolder(uri + '/intents/', 'intents',
				prefix + transition.intent.name + '_usersays_' + lan.languageAbbreviation + '.json')
		}
